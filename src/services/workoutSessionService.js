...
import * as workoutSessionRepository from "../repositories/workoutSessionRepository.js";
...
export async function getNextDay(userId, workoutPlanId, requesterRole){
...
export async function getSessions(userId, workoutPlanId, requesterRole){
    const plan = await loadOwnedPlan(workoutPlanId, userId, requesterRole);
    return workoutSessionRepository.findAllByUserAndPlan(userId, plan._id);
}

export async function completeDay(userId, requesterRole, {workoutPlan, day, date}){
...
export async function deleteSession(id, requesterId, requesterRole){
    assertValidId(id);
